...
| 场景 | 格式 | 示例 | 说明 |
|------|------|------|------|
| API 响应/请求 | `yyyyMMdd` | `20260101` | 统一使用 YYYYMMDD 格式，无分隔符 |
| 日志时间戳 | `yyyy-MM-dd HH:mm:ss.SSS` | `2026-01-01 12:30:45.123` | Logback 日志格式 |
| 数据库日期字段 | `DATE` / `DATETIME` | 数据库原生类型 | 存储时使用数据库类型 |
| 配置文件 | `yyyy-MM-dd` | `2026-01-01` | 人类可读格式 |

### 8.2 API 日期格式实现

```java
// ✅ 正确：API 响应使用 YYYYMMDD
...
// ✅ 正确：前端传参转换
@GetMapping("/orders")
public Result<OrderVO> queryOrders(@RequestParam String date) {
...
public class DateFormatUtils {
    ...
    public static LocalDate parseFromApi(String dateStr) {
        return LocalDate.parse(dateStr, API_FORMATTER);
    }
}
```
...
const formatDate = (date: string): string => {
  // API 返回 20260101，前端展示 2026-01-01
...
const toApiDate = (date: Date): string => {
  ...
  return `${year}${month}${day}`; // 20260101
};
```
