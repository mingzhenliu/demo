...
└── .knowledge/
    ...
    │
    ├── upstream/                # 上级知识库（Git Subtree 并行引入）
    │   ├── L0-enterprise/       # 企业级知识库（直接引用，确保即时可达）
    │   │   ├── constitution/    # 技术宪法（不可覆盖）
    │   │   ├── standards/       # 编码规范（可细化）
    │   │   ├── governance/      # 治理流程
    │   │   ├── ai-coding/       # AI 编码策略
...
### 5.4.2 上级知识库引用方案（Git Subtree 扁平模式）
...
| 特性 | 扁平模式（推荐） | 链式模式 |
|------|-----------------|----------|
| 路径深度 | `.knowledge/upstream/L0-enterprise/` | `.knowledge/upstream/L1/upstream/L0/` |
| L0 同步延迟 | 无（直接引用） | 需等 L1 先同步 |
| 版本控制 | L0/L1 独立更新 | L0 更新依赖 L1 |
| 维护成本 | 2 个 subtree | 1 个 subtree |
| 安全红线可达性 | ✅ 即时可达 | ⚠️ 可能延迟 |
...
> **重要**：扁平模式下，L2 仓库直接引用 L0 和 L1 两个知识库。这确保了：
> - **安全红线即时可达**：L0 的技术宪法、安全基线等不可覆盖规范可直接访问
> - **独立版本控制**：L0/L1 可按各自节奏更新，无传递延迟
> - **路径更短**：访问 L0 规范只需 `.knowledge/upstream/L0-enterprise/`
...
│  • 更新项目文档                                             │
│                                                             │
│  自顶向下继承 (Downward Inheritance) - 扁平引用             │
│  ─────────────────────────────────────────────              │
│                                                             │
│              ┌──────────────┐                               │
│              │  企业级 (L0)  │                               │
...
# 扁平引用模式配置
knowledge_sources:
  # L0 企业级知识库（直接引用，确保安全红线即时可达）
  enterprise:
    enabled: true
    path: ".knowledge/upstream/L0-enterprise"
    priority: 1  # 最高优先级，不可覆盖规范

  # L1 项目级知识库（直接引用，不含 L0）
  project:
    enabled: true
    path: ".knowledge/upstream/L1-project"
    priority: 2  # 次优先级，部分可覆盖
...
# 知识库同步配置
sync:
  auto_pull: false  # 是否自动拉取上游更新
  check_interval: "daily"  # 检查更新频率
  notify_on_update: true  # 上游更新时通知
