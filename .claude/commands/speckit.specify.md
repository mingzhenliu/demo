...
1. **加载知识库**：
   ...
   运行以下命令加载知识库配置：
   ```bash
   python3 .specify/scripts/python/load-knowledge.py specify
   ```

   脚本会输出：
   - ⚠️ 关键约束（术语词典、领域模型、仓库边界）
   - 📋 需要读取的文档列表
...
3. **在创建新分支之前检查现有分支**：
   ...
   d. 使用计算出的编号和短名称运行脚本 `python3 .specify/scripts/python/create-new-feature.py --json "$ARGUMENTS"`：
      - 传递 `--number N+1` 和 `--short-name "your-short-name"` 以及功能描述
      - 示例：`python3 .specify/scripts/python/create-new-feature.py --json --number 5 --short-name "user-auth" "Add user authentication"`
...
4. 加载 `.specify/templates/spec-template.md` 以了解所需的章节。
...
5. 遵循此执行流程：
    ...
    2. 从描述中提取关键概念
       识别：参与者、操作、数据、约束
    3. **术语一致性检查**：
       - 检查描述中的术语是否与 glossary.md 一致
       - 将非标准术语替换为标准术语
       - 新术语需明确标注并建议添加到词典
    4. **边界检查**：
       - 检查功能是否在本仓库职责范围内（参考 context.md）
       - 跨仓库依赖需明确标注
    5. 对于不清楚的方面：
       - 根据上下文和行业标准做出合理猜测
    ...
    7. 生成功能需求
       ...
       **实体定义需符合 domain-model.md**
    8. 定义成功标准
       创建可衡量的、与技术无关的结果
       包括定量指标（时间、性能、容量）和定性指标（用户满意度、任务完成）
       每个标准必须在没有实现细节的情况下可验证
    9. 识别关键实体（如果涉及数据）
       **必须与 domain-model.md 中的实体保持一致**
    10. 返回：成功（规范准备好进行规划）

6. 使用模板结构将规范写入 SPEC_FILE，用从功能描述（参数）派生的具体细节替换占位符，同时保留章节顺序和标题。

7. **规范质量验证**：在编写初始规范后，根据质量标准验证它：
...
8. 报告完成情况，包括分支名称、规范文件路径、检查清单结果，以及为下一阶段（`/speckit.clarify` 或 `/speckit.plan`）做好准备。
